...
def cluster_stocks_gmm(data, clusters):
    if data is None:
        return None
    ...
    clustering_model = GaussianMixture(n_components=clusters)
    clustering_model.fit(daily_movements_scaled)
    labels = clustering_model.predict(daily_movements_scaled)
    return labels
...
def main():
    ...
    # Cluster stocks using GMM
    labels = cluster_stocks_gmm(preprocessed_data, clusters)
